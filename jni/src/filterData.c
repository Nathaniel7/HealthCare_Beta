...
int F_checkStartBit(listNode* p, unsigned char* readBuff)
{
	...
	if (readBuff[0] == p->dev_maker_id[0] &&
		readBuff[1] == p->dev_maker_id[1]) {
		return 1;
	}
...
void F_filterData(listNode *p, unsigned char *readBuff, int count)
{
	...
	for(i = 0; i < 1; i++)
	{
		...
		data[i] = value[0]*100 + value[1];
//		__android_log_print(ANDROID_LOG_INFO, LOG_TAG, "Graph: %d*100 + %d = %d",value[0], value[1], data[i]);
